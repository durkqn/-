...
while not exit:
    ...
    K = (I[0]+l_arm*math.cos(30*math.pi/180),
         I[0]+l_arm*math.sin(30*math.pi/180))
    J = tup_r(J)
    K = tup_r(K)
    if try_num >= 3 :pygame.draw.line(screen, white, I, K, 3)
    if try_num >= 4 :pygame.draw.line(screen, white, I, K, 3)
    L = (I[0], I[1]+l_arm)
    if try_num >= 5 :pygame.draw.line(screen, white, I, L, 3)
    l_leg = round(l_arm * 1.5)
    M = (L[0]-l_arm*math.cos(60*math.pi/180),
         L[1]+l_arm*math.sin(60*math.pi/180))
    N = (L[0]+l_arm*math.cos(60*math.pi/180),
         L[1]+l_arm*math.sin(60*math.pi/180))
    M = tup_r(M)
    N = tup_r(N)
